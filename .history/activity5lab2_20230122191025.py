...
pp.pprint(np.all(bool_array)) # AND
pp.pprint(np.any(bool_array)) # OR

zero_arr = np.zeros((10, 10))
pp.pprint(zero_arr)
pp.pprint(np.any(zero_arr != 0)) # False
pp.pprint(np.any(zero_arr == 0)) # True

pp.pprint(arr.mean())
pp.pprint(np.median(arr))
pp.pprint(np.median(arr, axis = 0))
pp.pprint(np.median(arr, axis = 1))
pp.pprint(np.std(arr))
pp.pprint(np.std(arr, axis = 0))
pp.pprint(np.std(arr, axis = 1))
pp.pprint(np.std(arr, axis = -1))

# 3D array
threeD_arr = np.arange(27).reshape(3, 3, 3)
pp.pprint(threeD_arr)